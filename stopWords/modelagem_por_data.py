...
noticias=base.carrega_colecao_completo("brumadinhoinflux","colecao_processada")
...
def get_key(i):
    data=i['data'].split("/")
    new_data=to_int_str(data[1])+"/"+to_int_str(data[0])+"/"+to_int_str(data[2])
    return new_data
...
for i in dicio_data:
    arquivo.write("dia:"+str(i)+",N_noticias:"+str(len(dicio_data[i]))+"\n")
